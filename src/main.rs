...
async fn cmd_config(sub: ConfigCmd) -> Result<()> {
    match sub {
        ConfigCmd::Show => match ShepherdClient::connect().await {
            Ok(mut client) => match client.send_recv(&ClientMsg::GetConfig).await? {
        ...
        ConfigCmd::Slots { count } => match ShepherdClient::connect().await {
            ...
            Err(_) => {
                ...
                cfg.set_slots(count)?;
                println!(
                    "{} Slots saved to config as {} (daemon not running)",
                    "✔".green(),
                    slot_limit_label(cfg.slots)
                );
            }
        },
        ...
        ConfigCmd::Priority { dir, priority } => {
            ...
            match ShepherdClient::connect().await {
                ...
                Err(_) => {
                    let mut cfg = GlobalConfig::load()?;
                    cfg.set_project_priority(&project_dir, p)?;
                    println!(
                        "{} Default priority for '{}' saved as {} (daemon not running)",
                        "✔".green(),
                        project_dir,
                        p.label()
                    );
        ...
        ConfigCmd::Alias { dir, alias } => {
            ...
            match ShepherdClient::connect().await {
                Ok(mut client) => match client
                    .send_recv(&ClientMsg::SetProjectAlias {
                        project_dir: project_dir.clone(),
                        alias: alias.clone(),
                    })
                    .await?
                ...
                Err(_) => {
                    let mut cfg = GlobalConfig::load()?;
                    cfg.set_project_alias(&project_dir, &alias)?;
                    println!(
                        "{} Alias for '{}' saved as '{}' (daemon not running)",
                        "✔".green(),
                        project_dir,
                        alias
                    );
        ...
        ConfigCmd::ChildJobs { dir, jobs } => {
            ...
            match ShepherdClient::connect().await {
                ...
                Err(_) => {
                    let mut cfg = GlobalConfig::load()?;
                    cfg.set_project_child_jobs(&project_dir, jobs)?;
                    println!(
                        "{} child_jobs for '{}' saved as {} (daemon not running)",
                        "✔".green(),
                        project_dir,
                        jobs
                    );
